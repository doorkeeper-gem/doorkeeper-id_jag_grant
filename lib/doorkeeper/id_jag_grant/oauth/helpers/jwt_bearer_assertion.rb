  ...
  module IdJagGrant
    module OAuth
      module Helpers
        ...
        #
        # Ported from the official Doorkeeper PR #1861, adapted to read its
        # configuration from +Doorkeeper::IdJagGrant.configuration+.
        #
        ...
        module JwtBearerAssertion
          ...
          # @param config [Doorkeeper::IdJagGrant::Config] (default: the ID-JAG configuration)
          # @return [Result]
          def verify(assertion, client:, config: Doorkeeper::IdJagGrant.configuration)
          ...
          def resolve_key_candidates(assertion, config)
            ...
            return [] unless config.trusted_issuer.call(issuer)

            key_candidates(config.issuer_key.call(issuer, unverified_header["kid"]))
          end
          ...
          # Normalizes whatever `issuer_key` returned into a list of objects
          # `JWT::JWK.import` can build a key from: a PEM string is converted via
          # `OpenSSL::PKey.read`, a JWK `Hash`/`OpenSSL::PKey`/`JWT::JWK::KeyBase`
          ...
          # Verifies +assertion+ against a single candidate +key+, returning the
          # claims on success or nil on any failure (bad key, bad signature,
          # failed claim check).
          def attempt_decode(assertion, key, options)
            jwk = JWT::JWK.import(key)
            claims, = JWT.decode(assertion, jwk.verify_key, true, **options)
            claims
          rescue JWT::JWKError, JWT::DecodeError
            nil
          end
          private_class_method :attempt_decode

          # The `jwt` gem decode options enforcing algorithm allow-list,
          # audience, expiry/not-before (with clock skew), required claims and
          # replay protection.
          def decode_options(config)
            skew = config.clock_skew.to_i

            {
              algorithms: Array(config.allowed_algorithms),
              verify_aud: true,
              aud: config.audience_value,
          ...
          private_class_method :decode_options
